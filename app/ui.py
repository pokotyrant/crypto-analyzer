...
# Import core modules
from agent.orchestrator import CryptocurrencyOrchestrator
...
def get_orchestrator():
    return CryptocurrencyOrchestrator()
...
with st.sidebar:
    ...
    sessions = orchestrator.memory.get_sessions()
    ...
    try:
        all_logs = orchestrator.memory.get_all_guardrail_logs()
    ...
    st.markdown(f"- Gemini Key: {'✅ Active' if orchestrator.model else '⚠️ Fallback Mode'}")
    ...
    with col_suggestions:
        ...
        for name, query in coins_to_query:
            if st.button(f"🪙 {name}", use_container_width=True):
                orchestrator.run(st.session_state.session_id, query)
                st.rerun()
                
        st.markdown("---")
        st.markdown("**🛡️ Guardrail Demonstration:**")
        if st.button("❌ Test Buy Block", use_container_width=True):
            orchestrator.run(st.session_state.session_id, "Buy 5 Bitcoin using my portfolio")
            st.rerun()
        if st.button("⚠️ Test Portfolio Advice Block", use_container_width=True):
            orchestrator.run(st.session_state.session_id, "Make me an investment strategy for Ripple")
            st.rerun()
        st.markdown('</div>', unsafe_allow_html=True)

    with col_chat_main:
        # Load messages from SQLite Memory
        messages = orchestrator.memory.get_messages(st.session_state.session_id)
        ...
        if user_input:
            ...
            with st.chat_message("assistant", avatar="🤖"):
                with st.spinner("Agent Orchestrator consulting CoinGecko MCP Server and scraping sentiment..."):
                    # Run orchestrator
                    response = orchestrator.run(st.session_state.session_id, user_input)
            ...
            # Show trace for the last query
            st.rerun()
            
        # Display execution traces below the chat input if they exist
        tool_calls = orchestrator.memory.get_tool_calls(st.session_state.session_id)
        if tool_calls:
            st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    ...
    with col_dash_main:
        if dash_coin_id:
            with st.spinner(f"Loading data for '{dash_coin_id}'..."):
                ...
                chart_data_str = orchestrator.mcp_client.execute_tool(
            ...
            with st.spinner("Analyzing sentiment logs..."):
                # Check if news sentiment tool call is logged in memory
                # If not, let's fetch live sentiment statistics from fetcher
                news_articles = orchestrator.news_fetcher.fetch_crypto_news(dash_coin_id)
                sentiment_str = orchestrator._analyze_articles_sentiment(st.session_state.session_id, dash_coin_id, news_articles)
...
# TAB 3: SQLite Memory Inspector (For Kaggle Capstone grading/traceability)
with tab_grading:
    ...
    try:
        conn = orchestrator.memory._get_connection()
